...
$fl = new TemplatePower("./templates/mywantlist.tpl");

require("./header.php");

if(isset($login)&&$login==1 || isset($authenticated)&&$authenticated==1)
{
	$fl->assign("_ROOT.SITENAAM", $sitenaam." / My Wantlist");
	$selectid=get_uid($username);
	$query="SELECT b.id, b.title, b.publisher, p.name FROM fl_books b, fl_publishers p, fl_wanted c WHERE c.user='".$selectid."' AND c.book=b.id AND b.publisher=p.id AND b.approved!='0000-00-00 00:00:00' ORDER BY title";
	...
	else
	{
		...
		$fl->assign("FEEDBACK", "There are no items in your wantlist at this moment");
	}
}
else
{
